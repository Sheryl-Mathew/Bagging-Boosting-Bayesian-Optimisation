...
def bagging_based_on_tree_depth(train_data,train_label,test_data,test_label,depths,number_of_trees):
    accuracy_depth = []
    accuracies = {}
    for depth in depths:
        ...
        for number_of_tree in number_of_trees:
            model = BaggingClassifier(decision_tree,n_estimators=number_of_tree).fit(train_data, train_label)
...
def bagging_based_on_number_of_trees(train_data,train_label,test_data,test_label,depths,number_of_trees):
    accuracy_tree = []
    accuracies = {}
    for number_of_tree in number_of_trees:
        for depth in depths:
...
def print_accuracies_depth(accuracies_depth,type_of_data,number_of_trees):
    print("%s Accuracies for Number Decision Trees vs Depth of Tree: Bagging" %(type_of_data),file = f)
...
def print_accuracies_number_of_trees(accuracies_tree_number,type_of_data,depths):
    print("%s Accuracies for Depth of Tree vs Number of Decision Trees: Bagging" %(type_of_data),file = f)
...
def plot_curve_depth(train_accuracies,dev_accuracies,test_accuracies,depths,number_of_trees):
    ...
    for depth_of_tree in depths: 
        ...
        plt.ylabel("Accuracy (%)")
        plt.title("Depth of Tree %d vs Number of Decision Trees: Bagging" %(depth_of_tree))
        save_file_name = "Depth_of_Tree_%d_vs_Number_of_Decision_Trees_Bagging.png" %(depth_of_tree)
...
def plot_curve_number_of_trees(train_accuracies,dev_accuracies,test_accuracies,depths,number_of_trees):
    ...
    for number_of_tree in number_of_trees: 
        ...
        plt.ylabel("Accuracy (%)")
        plt.title("%d Decision Trees vs Depth of Tree: Bagging" %(number_of_tree))
        save_file_name = "%d_Decision_Trees_vs_Depth_of_Tree_Bagging.png" %(number_of_tree)
        plt.savefig(save_file_name)
        y_value_train = []
        y_value_dev = []
        y_value_test = []
...
def main():
    ...
    print("Bagging:",file = f)
    print(file = f)

    train_accuracies_depth = bagging_based_on_tree_depth(train_data_sk,train_label_sk,train_data_sk,train_label_sk,depths,number_of_trees)
    print_accuracies_depth(train_accuracies_depth,"Train",number_of_trees)
    dev_accuracies_depth = bagging_based_on_tree_depth(train_data_sk,train_label_sk,dev_data_sk,dev_label_sk,depths,number_of_trees)
    print_accuracies_depth(dev_accuracies_depth,"Development",number_of_trees)
    test_accuracies_depth = bagging_based_on_tree_depth(train_data_sk,train_label_sk,test_data_sk,test_label_sk,depths,number_of_trees)
    ...
    train_accuracies_tree_number = bagging_based_on_number_of_trees(train_data_sk,train_label_sk,train_data_sk,train_label_sk,depths,number_of_trees)
    print_accuracies_number_of_trees(train_accuracies_tree_number,"Train",depths)
    dev_accuracies_tree_number = bagging_based_on_number_of_trees(train_data_sk,train_label_sk,dev_data_sk,dev_label_sk,depths,number_of_trees)
    print_accuracies_number_of_trees(dev_accuracies_tree_number,"Development",depths)
    test_accuracies_tree_number = bagging_based_on_number_of_trees(train_data_sk,train_label_sk,test_data_sk,test_label_sk,depths,number_of_trees)
